...
Jxta_DiscoveryQuery * createDiscoveryQuery(){
  ...
  if(query == NULL && peerAdv == NULL) {
    if( query != NULL) JXTA_OBJECT_RELEASE(query);
    query = NULL;
    goto Common_Exit;
  }
...
Common_Exit:
    if( peerAdv != NULL) JXTA_OBJECT_RELEASE(peerAdv);
    if( attr != NULL) JXTA_OBJECT_RELEASE(attr);
    if( value != NULL) JXTA_OBJECT_RELEASE(value);
...
Jxta_DiscoveryQuery * createDiscoveryQuery_1(){
  ...
  if( peerAdv == NULL) return query;

  JXTA_OBJECT_SHARE( peerAdv);
  query = jxta_discovery_query_new_1(DISC_PEER,
				     "The Attribute",
				     "The Value",
				     20,
				     peerAdv);
...
boolean checkDiscoveryQuery(Jxta_DiscoveryQuery * query){
  ...
  if(comp == NULL ||
     strcmp("The Peer Advertisement content", jstring_get_string(comp)) != 0){
     result = FALSE;
     goto Common_Exit;
  }
  ...
  if(comp == NULL ||
     strcmp("The Attribute", jstring_get_string(comp)) != 0){
     result = FALSE;
     goto Common_Exit;
  }
  JXTA_OBJECT_RELEASE(comp);
  ...
  if(comp == NULL ||
     strcmp("The Value", jstring_get_string(comp)) != 0){
     result = FALSE;
     goto Common_Exit;
  }
  JXTA_OBJECT_RELEASE(comp);
...
Common_Exit:
    if( comp != NULL) JXTA_OBJECT_RELEASE(comp);
   return result;
}
...
test_jxta_discovery_query_new(void) {
  ...
  if( !checkDiscoveryQuery(query) ) result = FALSE;
...
test_jxta_discovery_read_write(void) {
  ...
  if( JXTA_SUCCESS != 
      jxta_discovery_query_parse_charbuffer(query,
...
Common_Exit:
    if( doc != NULL) JXTA_OBJECT_RELEASE(doc);
    if( query != NULL) jxta_discovery_query_free(query);
    return result;
}
