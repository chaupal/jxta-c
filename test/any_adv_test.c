...
int main(int argc, char **argv)
{
    ...
    if (argc != 2) {
        ...
        printf("note: a file containing many of these enclosed in a document of" " any name will work too (see many.xml)\n");
        return -1;
    }
    ...
    jxta_advertisement_register_global_handler("jxta:PA", (const JxtaAdvertisementNewFunc) jxta_PA_new);
    jxta_advertisement_register_global_handler("jxta:PGA", (const JxtaAdvertisementNewFunc) jxta_PGA_new);
    jxta_advertisement_register_global_handler("jxta:MIA", (const JxtaAdvertisementNewFunc) jxta_MIA_new);
    ...
    if (advs == NULL) {
        ...
        for (i = 0; i < sz; i++) {
            ...
            jxta_vector_get_object_at(advs, JXTA_OBJECT_PPTR(&adv), i);
